...
def main():
    ...
    for lam in l2_lambdas:
        model = SoftmaxModel(l2_reg_lambda=lam)
    ...
    for lam, val_history in zip(l2_lambdas, val_histories):
        utils.plot_loss(val_history["accuracy"], label=f"λ={lam}")

    plt.xlabel("Number of Training StepsX")
